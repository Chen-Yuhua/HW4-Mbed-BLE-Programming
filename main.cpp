...
const static char DEVICE_NAME[] = "Group9";
...
public:
    HeartrateDemo(BLE &ble, events::EventQueue &event_queue) :
        ...
        // Update : magneto service
        magneto_uuid(GattService::UUID_ENVIRONMENTAL_SERVICE),
        magneto_x_value(0),
        magneto_y_value(0),
        magneto_z_value(0),
        magneto_service(ble),
...
private:
    ...
    void update_sensor_value()
    {
        ...
        printf("Heartrate value : %d\n", _heartrate_value);
        
        _heartrate_service.updateHeartRate(_heartrate_value);
    }

    // Update : magneto service
    void update_magneto_value()
    {        
        int16_t pDataXYZ[3];
        BSP_MAGNETO_GetXYZ(pDataXYZ);
        magneto_x_value = pDataXYZ[0];
        magneto_y_value = pDataXYZ[1];
        magneto_z_value = pDataXYZ[2];
        printf("Magneto value : x = %d, y = %d, z = %d\n", magneto_x_value, magneto_y_value, magneto_z_value);

        magneto_service.updateMagneto(pDataXYZ, 0);
    }
...
private:
    ...
    // Update : magneto service
    UUID magneto_uuid;
    int16_t magneto_x_value;
    int16_t magneto_y_value;
    int16_t magneto_z_value;
    MagnetoService magneto_service;
